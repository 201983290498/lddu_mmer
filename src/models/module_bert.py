...
class TCNBertModel(PreTrainedModel):
    """Visual model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config):
        super(TCNBertModel, self).__init__(config)
        word_embeddings = nn.Linear(config.vocab_size, config.hidden_size) # 去掉了位置编码
        layerNorm = LayerNorm(config.hidden_size, eps=1e-12)
        dropout = nn.Dropout(config.hidden_dropout_prob)
        self.embeddings = nn.Sequential(word_embeddings, layerNorm, dropout)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
        self.apply(self.init_weights)
