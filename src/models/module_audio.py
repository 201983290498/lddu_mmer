...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.3):
        ...
        self.conv1 = weight_norm(nn.Conv1d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation))  # 1维度卷积
        self.relu1 = nn.ReLU()
        self.dropout1 = nn.Dropout(dropout)
        self.conv2 = weight_norm(nn.Conv1d(n_outputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation)) # 1维度卷积
        self.relu2 = nn.ReLU()
        self.dropout2 = nn.Dropout(dropout)
        self.downsample = nn.Conv1d(n_inputs, n_outputs, 1) if n_inputs != n_outputs else None # 下采样
        self.relu = nn.ReLU()
        self.init_weights()
        self.net = nn.Sequential(self.conv1, self.relu1, self.dropout1, self.conv2, self.relu2, self.dropout2)

    def init_weights(self):
        self.conv1.weight.data.normal_(0, 0.01) # 权重参数
    ...
    def forward(self, x):
        out = self.net(x)
        res = x if self.downsample is None else self.downsample(x) # 直接下采样使用残差结构
...
class TemporalConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=3, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i-1] # 多吃度卷积
...
class TCNAudioEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config):
        super(TCNAudioEmbeddings, self).__init__()
...
class TCNAudioModel(PreTrainedModel):
    """Audio model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config):
        super(TCNAudioModel, self).__init__(config)
        self.embeddings = TCNAudioEmbeddings(config)
        self.encoder = AudioEncoder(config)
        self.pooler = AudioPooler(config)
        self.apply(self.init_weights)
